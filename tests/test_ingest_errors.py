...
def test_detect_source_family_partial_guardduty() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="Partial GuardDuty match but missing fields: 'detail-type' must be 'GuardDuty Finding'",
    ):
        detect_source_family(payload)
...
def test_detect_source_family_partial_securityhub() -> None:
    ...
    with pytest.raises(
        ValueError, match="Partial SecurityHub match but missing fields: missing 'ProductArn'"
    ):
        detect_source_family(payload)
...
def test_detect_source_family_partial_cloudtrail() -> None:
    ...
    with pytest.raises(
        ValueError, match="Partial CloudTrail match but missing fields: eventName, userIdentity"
    ):
        detect_source_family(payload)
